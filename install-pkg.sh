...
if [ $dis == "Raspbian" ] || [ $dis == "Debian" ]; then
   ...
   if [ $ver == "11" ]; then
    ...
    echo "Instalacja pakietów dla Remote LCD2004 na bazie Debian 11"
    echo ""
    sudo apt-get update
    ...
    echo ""
    echo "Kopiowanie remotelcd.service do /lib/systemd/system/"
    sudo cp /opt/fmpoland/remotelcd/remotelcd.service /lib/systemd/system/
    echo ""
   ...
   if [ $ver == "12" ]; then
    ...
    echo "Instalacja pakietów dla Remote LCD2004 na bazie Debian 12"
    echo ""
    sudo apt-get update
    sudo apt install -y python3 python3-pip python3-dev python3-numpy python3-watchdog python3-requests
